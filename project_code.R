...
#after looping through the values of k set in kvalue we output the 
#value of k, the mean of the prediction error rate, and the table
#of the predictions made.


#Importance of scaling data
#scaling data is essential to making both training and test sets 
#on the same scale
## using scaled data 
set.seed(11)
train = sample(n, 0.8*n)
...
#now we make the predictions for each k in kvalue
#as we can see from the results our scaled outputs 
...
#After comparing the best KNN and SVM models we fit the entire dataset to the optimal model
